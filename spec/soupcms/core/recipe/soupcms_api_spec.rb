...
describe SoupCMS::Core::Recipe::SoupCMSApi do
  ...
  context 'popular posts' do
    ...
    let(:posts) do
      stub_request(:get, /posts\?fields%5B%5D=title&filters%5B%5D=tags&tags=popular$/).to_return({body: read_files('posts/first-post', 'posts/second-post')})
      recipe.execute
    end
  ...
  context 'eval values in recipe' do
    ...
    let(:posts) do
      stub_request(:get, /posts\?filters%5B%5D=tags&tags=soupcms-test-append$/).to_return({body: read_files('posts/first-post', 'posts/second-post')})
      recipe.execute
    end
