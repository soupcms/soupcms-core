...
describe SoupCMS::Core::Api::Service do
  ...
  context 'published' do
    ...
    context 'find with filters' do
      it 'should return parsed JSON objects for 200 response' do
        stub_request(:get, /posts\?filters%5B%5D=tags&tags=popular$/).to_return({body: read_files('posts/first-post', 'posts/second-post')})
      ...
      it 'should return empty array for 404 response' do
        stub_request(:get, /posts\?filters%5B%5D=tags&tags=popular$/).to_return(status: 404)
        docs = service.find('posts', {tags: 'popular'})
        expect(docs.size).to eq(0)
      end

      it 'should send appropriate request without quotes for integer value' do
        stub_request(:get, /posts\?filters%5B%5D=rank&rank=100$/).to_return({body: read_files('posts/first-post', 'posts/second-post')})
        docs = service.find('posts', {rank: 100})
        expect(docs.size).to eq(2)
      end

      it 'should send appropriate request for array value' do
        stub_request(:get, /posts\?filters%5B%5D=tags&tags%5B%5D=liked&tags%5B%5D=popular$/).to_return({body: read_files('posts/first-post', 'posts/second-post')})
        docs = service.find('posts', {tags: %w(popular liked)})
        expect(docs.size).to eq(2)
      end

      it 'should handle multiple filter parameters' do
        stub_request(:get, /posts\?filters%5B%5D=rank&filters%5B%5D=tags&rank=100&tags=popular$/).to_return({body: read_files('posts/first-post', 'posts/second-post')})
  ...
  context 'draft' do
    ...
    it 'should include draft when in request for find with filters' do
      stub_request(:get, /posts\?filters%5B%5D=tags&include=drafts&tags=popular$/).to_return({body: read_files('posts/first-post', 'posts/second-post')})
      docs = service.find('posts', {tags: 'popular'})
      expect(docs.size).to eq(2)
    end
